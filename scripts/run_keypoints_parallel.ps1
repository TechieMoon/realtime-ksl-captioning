...
$mainLog = Join-Path "logs" "keypoints_parallel_$timestamp.log"
...
function Start-Shards {
    ...
    for ($i = 0; $i -lt $NumShards; $i++) {
        ...
        Write-Log "  shard $i pid=$($p.Id) manifest=$manifest skipped=manifests/$ManifestPrefix`_shard_$i.skipped.json"
        $processes += $p
    }
