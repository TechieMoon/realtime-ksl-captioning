...
def main() -> None:
    args = parse_args()
    extractor_config = HolisticExtractorConfig(
        task_model_path=args.task_model_path,
        target_fps=args.target_fps,
        max_frames=args.max_frames,
    )
    extractor = HolisticKeypointExtractor(extractor_config)
    ...
    try:
        for sample in tqdm(
            ...
            try:
                ...
                try:
                    ...
                    tqdm.write(
                        "warning: failed to close MediaPipe landmarker after "
                        f"video error ({type(close_exc).__name__}: {close_exc})"
                    )
                extractor = HolisticKeypointExtractor(extractor_config)
                continue
        ...
        try:
            ...
            tqdm.write(
                "warning: failed to close MediaPipe landmarker "
