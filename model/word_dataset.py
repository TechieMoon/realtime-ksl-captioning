...
def _candidate_video_paths(split_root: Path, video_name: str) -> list[Path]:
    ...
    if split_root.name.startswith("1."):
        ...
        candidates.append(
            split_root / f"[?먯쿇]{source_id:02d}_real_word_video" / real_dir / video_name
        )
    else:
        candidates.append(
            split_root / "[?먯쿇]01_real_word_video" / "WORD" / real_dir / video_name
        )
    return candidates
...
def iter_samples(
    ...
    label_root = split_root / "[?쇰꺼]01_real_word_morpheme" / "morpheme"
